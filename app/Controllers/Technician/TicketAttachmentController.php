...
class TicketAttachmentController extends Controller
{
    ...
    public function download(?array $data): void
    {
        ...
        if (!file_exists($attachment->getFilePath())) {
            ...
            redirect("/tecnico/chamados/" . $attachment->getTicketId() . "/comentarios");
            return;
        }
    ...
    {
        Auth::requirePermission(Permission::DELETE_ANY_ATTACHMENT);

        $ticketId     = (int)($data["ticket_id"] ?? 0);
        $attachmentId = (int)($data["id"] ?? 0);

        $this->validateCsrfToken($data, "/tecnico/chamados/{$ticketId}/anexos/excluir/{$attachmentId}");
